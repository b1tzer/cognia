"""Cognia 记忆层（长期 Store 领域读写）。

职责边界（plan §3.4、clarifications Q6）：
- Checkpointer：短期会话状态，按 `thread_id` 恢复（一次会话）。
- Store：长期认知状态，按 `user_id` 隔离，跨会话持久化。
  - `("observation", user_id)`：AI 观察样本（只追加，BKT 融合后定级）。
  - `("profile", user_id)`：稳定偏好（沟通风格 / 语言）。
  - `("knowledge_model", user_id)`：冻结的知识模型（load-or-build）。
  - `("concept", user_id)`：全局概念注册表（跨对话概念身份归一）。

匿名 `user_id` 通过 runtime context（`config["configurable"]["user_id"]`）注入，
绝不塞进 State（宪法 §5）。前端生成 UUID 并持久化，后端仅作隔离映射。

本模块只放「领域存储逻辑」（observation / profile / knowledge_model / concept
的 store 读写），逻辑函数接受 `store` 参数、不自行建连接，便于测试用
`InMemoryStore` 注入。生产工厂（连接池 / checkpointer / store）已迁至
`cognia.infra.store`，这里 re-export 以兼容既有调用方（`memory.get_pool()` 等）。
"""
