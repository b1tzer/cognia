...
# 长会话上下文管理（滑动窗口 + 摘要压缩）
#
# 落地行业主流 ConversationSummaryBufferMemory（hybrid）模式：
# 滚动摘要 + 最近 N 轮 verbatim，替代旧的「概念级 3 轮短栈」主动遗忘。
# - 组装 prompt 时，注入「滚动摘要」+「最近 KEEP_RECENT 条 verbatim 消息」；
# - 对话消息数超过 SUMMARY_THRESHOLD 时，把超出窗口的旧对话压缩进滚动摘要。
# ---------------------------------------------------------------------------
# 组装 prompt 时保留的最近 verbatim 消息条数（超出部分压入摘要）
CONTEXT_KEEP_RECENT = 10
# 消息数超过该阈值才触发摘要（把超出窗口的旧对话压缩进滚动摘要）
CONTEXT_SUMMARY_THRESHOLD = 16

# --------------------------------------------------------------------------
# 用户习惯/偏好记忆（需求 #71 需求3）
#
# 把 learner_profile.preferences_json 接上数据：累计 N 轮对话后，由 LLM
# 从对话中抽取用户交互风格偏好，merge 进长期记忆；教学决策与回复时读回注入。
# ---------------------------------------------------------------------------
# 累计多少个对话轮次触发一次偏好抽取（批量抽取省 token，不逐轮调用）
PREF_EXTRACT_THRESHOLD = 10
